...
import { FileUpload } from './FullUpload';
import { ChatMessage as ChatMessageComponent } from'./ChatMessage';
import { ChatMessage, FileType, FileUpload as FileUploadType } from './type';
...
function App() {
  ...
  const handleFileSelect = async (file: File, type: FileType) => {
    ...
    setCurrentFile({ file, type, preview });
  };
  ...
  const sendQuery = async (query: string) => {
    try {
      ...
      const response = await fetch('https://af2e-34-143-188-208.ngrok-free.app/get_result/', {
        method: 'POST',
        body: formData,
      });
