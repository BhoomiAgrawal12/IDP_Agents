...
BASE_VIDEO_DIR = "./video_data"
BASE_MIXED_DIR = "./mixed_data"
os.makedirs(BASE_VIDEO_DIR, exist_ok=True)
os.makedirs(BASE_MIXED_DIR, exist_ok=True)
...
def video_to_images(video_path: str, output_folder: str):
    clip = VideoFileClip(video_path)
...
def convert_wav_to_flac(input_wav: str, output_flac: str):
    command = ["ffmpeg", "-y", "-i", input_wav, "-c:a", "flac", output_flac]
...
async def process_video(request: VideoDownloadRequest):
    video_url = request.url
    filepath = os.path.join(BASE_VIDEO_DIR, "input_vid.webm")  # Assuming webm format from yt_dlp
    output_audio_wav = os.path.join(BASE_MIXED_DIR, "output_audio.wav")
    output_audio_flac = os.path.join(BASE_MIXED_DIR, "output_audio.flac")

    # Download video
    metadata = download_video(video_url, BASE_VIDEO_DIR)

    # Convert to images
    video_to_images(filepath, BASE_MIXED_DIR)

    # Extract audio
    video_to_audio(filepath, output_audio_wav)

    # Convert to FLAC
    convert_wav_to_flac(output_audio_wav, output_audio_flac)

    # Transcribe audio
    transcription = transcribe_audio(output_audio_flac)

    return {
        "Video_Metadata": metadata,
        "Transcription": transcription,
        "Frames_Saved_In": BASE_MIXED_DIR
    }
